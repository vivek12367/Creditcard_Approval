...
st.markdown(
    ...
    .stTextInput>div>div>input, .stSelectbox>div>div>select, .stRadio>div {
        padding: 8px;
        border-radius: 8px;
    }
    </style>
...
train_original = pd.read_csv(
    "/Users/jahnavipragada/Downloads/train.csv"
)

test_original = pd.read_csv(
    "/Users/jahnavipragada/Downloads/test.csv"
)

full_data = pd.concat([train_original, test_original], axis=0)

full_data = full_data.sample(frac=1).reset_index(drop=True)
# ✅ UI: Credit Card Prediction App
st.markdown(
    "<h1 style='text-align: center; color: #FFA500;'>💳 Cred Guru</h1>",
    unsafe_allow_html=True
)

st.write(
    """
    📝 Fill in the details below and click **Predict** to check if your application will be approved.
    """
)
